...
describe('build-index output', () => {
	...
	it('generates valid JSON files for project sessions', () => {
		...
		const filePath = path.join(DATA_DIR, 'projects', project.dirName, `${session.sessionId}.json`)
		expect(fs.existsSync(filePath)).toBe(true)
	...
	it('indexes file history conversations', () => {
		...
		const filePath = path.join(DATA_DIR, 'file-history', `${entry.conversationId}.json`)
		expect(fs.existsSync(filePath)).toBe(true)
		const detail: FileHistoryDetail = JSON.parse(fs.readFileSync(filePath, 'utf-8'))
	...
	it('indexes history entries sorted newest first', () => {
		...
		for (let i = 1; i < index.history.length; i++) {
			expect(index.history[i - 1].timestamp).toBeGreaterThanOrEqual(index.history[i].timestamp)
		}
	})
})
