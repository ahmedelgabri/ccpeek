...
Migration 18 assigns legacy claims version zero, so the first reparse after upgrade treats each observed request as a newer interpretation. At that transition, and at later parser-version bumps even for unrelated adapter changes, a poorer surviving copy can replace figures retained from a pruned richer copy in canonical totals; token maxima and reported costs are not inherited across versions. The older interpretation remains stored, but no longer determines those totals.
...
If the latest run is still recorded as `running`, readiness remains at 503 (`index-unfinished`), even when this server has completed startup or uses `--skip-index`. That row may belong to an active process or a pass that was killed before recording completion; it is not proof of live progress. The UI shows the unfinished state and polls for completion. If no indexing process is active, run `ccpeek ingest` to retry. A subsequent successful, complete pass clears this readiness hold; merely releasing the maintenance lock or repairing derived data does not finish an interrupted run.
