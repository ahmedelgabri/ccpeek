...
const tlContainer = document.getElementById("tool-timeline");
if (tlContainer) {
  const data: ToolData[] = JSON.parse(tlContainer.dataset.timeline || "[]");
  if (data.length > 0) {
    const colors = [
      "#8b5cf6",
      "#06b6d4",
    ...
    const legend = toolNames
      .map((name) => {
        ...
        return `<span class="inline-flex items-center gap-1 text-xs text-slate-500"><span class="inline-block w-2 h-2 rounded-sm" style="background:${color}"></span>${name}</span>`;
      })
      .join(" ");
