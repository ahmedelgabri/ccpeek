...
// Shape of raw JSONL lines from Claude's conversation logs
...
function indexProjects(): ProjectEntry[] {
	...
	for (const dirName of projectDirs) {
		...
		if (fs.existsSync(sessionsIndexPath)) {
			try {
				sessionsIndex = JSON.parse(fs.readFileSync(sessionsIndexPath, 'utf-8'))
			} catch {
				// ignore parse errors
			}
		}
		...
		for (const jsonlFile of jsonlFiles) {
			...
			const lines = readJsonl<RawJsonlLine>(jsonlPath)
			...
			for (const line of lines) {
				...
				if (obj.type === 'user' || obj.type === 'assistant' || obj.type === 'system') {
					...
					messages.push({
						...
						uuid: obj.uuid ?? '',
						message: obj.message ?? { role: obj.type, content: '' },
			...
			fs.writeFileSync(path.join(outDir, `${sessionId}.json`), JSON.stringify(messages))

			// Build session metadata
			const indexEntry = sessionsIndex?.entries?.find((e) => e.sessionId === sessionId)
...
function indexFileHistory(): FileHistoryEntry[] {
	...
	for (const conversationId of conversationDirs) {
		...
		fs.writeFileSync(path.join(outDir, `${conversationId}.json`), JSON.stringify(detail))
...
function indexHistory(): HistoryEntry[] {
	...
	const lines = readJsonl<HistoryEntry>(historyPath)
	return lines.toSorted((a, b) => b.timestamp - a.timestamp)
}
